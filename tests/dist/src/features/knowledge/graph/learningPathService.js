...
exports.getClinicalCategory = getClinicalCategory;
exports.getFallbackComparison = getFallbackComparison;
exports.scoreEntity = scoreEntity;
exports.getTopEntitiesOfType = getTopEntitiesOfType;
exports.generateLearningPath = generateLearningPath;
const index_1 = require("../index");
const entityRelationships_1 = require("./entityRelationships");
const comparisonRegistry_1 = require("../comparisons/comparisonRegistry");
const collectionsRegistry_1 = require("../collections/collectionsRegistry");
...
function getClinicalCategory(entity) {
    const id = entity.id;
    ...
    for (const col of collectionsRegistry_1.CURATED_COLLECTIONS) {
        if (col.entityIds.includes(entity.id)) {
            if (col.id === "col-thyroid")
                return "thyroid";
            if (col.id === "col-digestive")
                return "gastrointestinal";
            if (col.id === "col-respiratory")
                return "respiratory";
            if (col.id === "col-skin")
                return "dermatology";
            if (col.id === "col-musculoskeletal")
                return "musculoskeletal";
            if (col.id === "col-womens")
                return "urology";
        }
    }
    return "general";
}
function getFallbackComparison(currentEntity, category) {
    const allEntities = (0, index_1.getAllKnowledgeEntities)();
    // 1. Try to find a comparison that explicitly features this entity
    const directComp = comparisonRegistry_1.COMPARISONS.find(c => c.entity1Id === currentEntity.id || c.entity2Id === currentEntity.id);
    if (directComp)
        return `/knowledge/compare/${directComp.slug}`;
    // 2. Try to find a comparison where at least one entity is of the same clinical category
    const categoryComp = comparisonRegistry_1.COMPARISONS.find(c => {
        const e1 = allEntities.find(x => x.id === c.entity1Id);
        const e2 = allEntities.find(x => x.id === c.entity2Id);
        if (e1 && getClinicalCategory(e1) === category)
            return true;
        if (e2 && getClinicalCategory(e2) === category)
            return true;
        return false;
    });
    if (categoryComp)
        return `/knowledge/compare/${categoryComp.slug}`;
...
function scoreEntity(candidate, source) {
    let score = 0;
    // Direct relationship in graph (incoming or outgoing)
    const directRel = entityRelationships_1.KNOWLEDGE_RELATIONSHIPS.find(rel => (rel.source === source.id && rel.target === candidate.id) ||
    ...
    if (sourceCat === "thyroid" && candidateCat === "thyroid") {
        if (candidate.slug === "tsh")
            score += 200;
        if (candidate.slug === "ft4" || candidate.slug === "free-t4")
            score += 150;
        if (candidate.slug === "ft3" || candidate.slug === "free-t3")
            score += 100;
    }
    // Specialty match
    ...
    const sourceNeighbors = new Set(entityRelationships_1.KNOWLEDGE_RELATIONSHIPS.filter(rel => rel.source === source.id || rel.target === source.id)
        .map(rel => rel.source === source.id ? rel.target : rel.source));
    const candidateNeighbors = entityRelationships_1.KNOWLEDGE_RELATIONSHIPS.filter(rel => rel.source === candidate.id || rel.target === candidate.id)
...
function getTopEntitiesOfType(currentEntity, type, count, excludeIds) {
    const allEntities = (0, index_1.getAllKnowledgeEntities)();
...
function getShortTitle(titleObj) {
...
function getContextAwareDescription(entity, parentEntity) {
...
function generateLearningPath(currentEntity, mode) {
    const steps = [];
    const excludeIds = new Set([currentEntity.id]);
    ...
    const getUrl = (entity) => {
        return (0, index_1.getEntityUrl)(entity.entityType, entity.slug);
    };
    // 1. DISEASE PATHS
