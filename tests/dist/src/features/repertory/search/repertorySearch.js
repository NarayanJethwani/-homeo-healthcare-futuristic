...
exports.RepertorySearch = void 0;
...
class RepertorySearch {
    /**
     * Performs a synonym-expanded multi-field text search on rubrics.
     */
    static async searchRubrics(queryText, filters, boostRelationships = false, enableSemanticExpansion = false) {
        const rubrics = await repertoryDb_1.repertoryRepository.getRubrics(filters);
        ...
        const scoredList = rubrics.map(rubric => {
            ...
            queryTokens.forEach(token => {
                ...
                const syns = repertoryData_1.SEARCH_SYNONYMS[token] || [];
                syns.forEach(s => {
                ...
                tokenTerms.forEach(term => {
                    ...
                    const remMatch = rubric.relatedRemedies.some(r => r.remedyId.toLowerCase() === term || hasWord(r.remedyName.toLowerCase(), term));
                    if (remMatch) {
        ...
        if (enableSemanticExpansion && scoredList.some(item => item.score >= 40)) {
            ...
            for (const match of topMatches) {
                const neighbors = await repertoryGraph_1.RepertoryGraph.getSemanticNeighbours(match.rubric.rubricId);
     ...
     */
    static async parseAIIntakeText(intakeText) {
        const result = {
        ...
        const phrases = intakeText
            ...
            .map(p => p.trim())
            .filter(p => p.length > 5);
        const rubricMatchesMap = new Map();
        ...
        for (const match of result.matchedRubrics) {
            const rub = await repertoryDb_1.repertoryRepository.getRubricById(match.rubricId);
            if (rub) {
        ...
        for (const match of result.matchedRubrics) {
            const rub = await repertoryDb_1.repertoryRepository.getRubricById(match.rubricId);
            if (rub)
                matchedCategories.add(rub.category);
        }
...
exports.RepertorySearch = RepertorySearch;
