...
exports.RepertoryGraph = void 0;
const repertoryDb_1 = require("../database/repertoryDb");
class RepertoryGraph {
    /**
    ...
    static async ensureInitialized() {
        ...
        for (const t of triples) {
            ...
            if (!this.nodes.has(t.objectId)) {
                let nodeType = 'rubric';
                if (t.predicate === 'suggestsMiasm')
                    nodeType = 'miasm';
                else if (t.predicate === 'mapsToDisease')
                    nodeType = 'disease';
                else if (t.predicate === 'belongsToOrgan')
                    nodeType = 'organ_system';
                else if (t.predicate === 'differentiates')
                    nodeType = 'remedy';
    ...
    static addNode(id, type, label) {
        if (!this.nodes.has(id)) {
            this.nodes.set(id, { id, type, label });
        }
    }
    static addEdge(sourceId, targetId, type, weight) {
     ...
     */
    static async registerRelationship(sourceId, type, targetId, weight = 1.0) {
     ...
     */
    static async traverseGraph(startId, maxDepth = 2) {
        await this.ensureInitialized();
     ...
     */
    static async getSemanticNeighbours(rubricId) {
        await this.ensureInitialized();
        if (this.semanticNeighboursCache.has(rubricId)) {
            return this.semanticNeighboursCache.get(rubricId);
        }
        const rubric = await repertoryDb_1.repertoryRepository.getRubricById(rubricId);
        if (!rubric)
            return [];
        ...
        for (const [neighId, nodeInfo] of traversal.entries()) {
            ...
            const otherRubric = await repertoryDb_1.repertoryRepository.getRubricById(neighId);
            if (otherRubric) {
     ...
     */
    static async getRelatedRubrics(rubricId) {
        const triples = await repertoryDb_1.repertoryRepository.getTriples();
     ...
     */
    static async getSuggestedMiasms(rubricId) {
        const triples = await repertoryDb_1.repertoryRepository.getTriples();
        return triples
     ...
     */
    static async getDiseasesMapped(rubricId) {
        const triples = await repertoryDb_1.repertoryRepository.getTriples();
        return triples
     ...
     */
    static async getDifferentiatingRubrics(remedyA, remedyB) {
        const allRubrics = await repertoryDb_1.repertoryRepository.getRubrics();
        const diffs = [];
        for (const rub of allRubrics) {
            ...
            if (Math.abs(gradeA - gradeB) >= 2 || (gradeA >= 3 && gradeB === 0) || (gradeB >= 3 && gradeA === 0)) {
                const diffWeight = Math.max(covA?.clinicalExperienceWeight || 0.5, covB?.clinicalExperienceWeight || 0.5) * Math.abs(gradeA - gradeB);
                diffs.push({
                    rubric: rub,
                    gradeA,
                    gradeB,
...
exports.RepertoryGraph = RepertoryGraph;
RepertoryGraph.isInitialized = false;
RepertoryGraph.nodes = new Map();
RepertoryGraph.adjacencyList = new Map();
// Memoization Caches
RepertoryGraph.semanticNeighboursCache = new Map();
RepertoryGraph.pathCache = new Map();
RepertoryGraph.traversalCache = new Map();
