...
exports.MiasmaticEngine = void 0;
class MiasmaticEngine {
    /**
     * Evaluates active symptoms and rubric weights to determine the dominant miasmatic currents.
     */
    static analyzeMiasms(rubrics, symptoms) {
        const miasmaticTotals = {
            Psora: 0,
            Sycosis: 0,
            Syphilis: 0,
            Tubercular: 0,
        ...
        for (const rub of rubrics) {
            if (rub.miasmaticWeight) {
                ...
                Object.entries(rub.miasmaticWeight).forEach(([miasm, weight]) => {
                    ...
                    miasmaticTotals[miasm] += scoreContribution;
        ...
        const rankedMiasms = Object.entries(miasmaticTotals)
            .map(([name, weight]) => ({ name: name, weight }))
        ...
        const confidence = totalPoints > 0
            ...
            : 50;
        const reasoning = `Case exhibits a primary ${primaryMiasm} load (${Math.round(rankedMiasms[0].weight)} points)${secondaryMiasm ? ` with secondary ${secondaryMiasm} tendencies (${Math.round(rankedMiasms[1].weight)} points)` : ''}.`;
        // Map miasm fit per remedy (Ars = Syphilitic, Nux = Psoric/Sycotic, Lyc = Sycotic, Sulph = Psoric, Puls = Sycotic)
        const remedyMiasmaticFit = {
...
exports.MiasmaticEngine = MiasmaticEngine;
