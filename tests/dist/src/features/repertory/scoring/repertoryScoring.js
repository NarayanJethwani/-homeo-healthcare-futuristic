...
exports.RepertoryScoring = void 0;
...
const constitutionalEngine_1 = require("./constitutionalEngine");
const miasmaticEngine_1 = require("./miasmaticEngine");
class RepertoryScoring {
    /**
     * Performs multi-factorial weighted repertorization scoring on selected symptoms.
     */
    static async calculateRepertorization(symptoms) {
        const result = {
            topRemedies: [],
        ...
        for (const sym of symptoms) {
            ...
            if (rub.category === 'Thermal State' || rub.thermalState)
                hasThermalSelected = true;
            if (rub.thirstPattern && rub.thirstPattern !== 'normal')
                hasThirstSelected = true;
            if (rub.category === 'Modalities' || (rub.modalities && rub.modalities.length > 0) || (rub.aggravations && rub.aggravations.length > 0) || (rub.ameliorations && rub.ameliorations.length > 0))
                hasModalitiesSelected = true;
            if (rub.category === 'Female / Menses' || (rub.organSystem && (rub.organSystem.toLowerCase().includes('menses') || rub.organSystem.toLowerCase().includes('female'))))
                hasMensesSelected = true;
            if (rub.category === 'Sleep')
                hasSleepSelected = true;
            if (rub.category === 'Etiology / Causation')
                hasEtiologySelected = true;
            // Weight calculation
        ...
        const constProfile = constitutionalEngine_1.ConstitutionalEngine.analyzeConstitution(activeRubrics, symptoms);
        const miasmProfile = miasmaticEngine_1.MiasmaticEngine.analyzeMiasms(activeRubrics, symptoms);
        ...
        for (const sym of symptoms) {
            ...
            switch (rub.category) {
                case 'Etiology / Causation':
                    categoryMultiplier = scoringConfig_1.CLINICAL_SCORING_CONFIG.etiology;
                    break;
                case 'Mental & Emotional':
                    categoryMultiplier = scoringConfig_1.CLINICAL_SCORING_CONFIG.mentalGenerals;
                    break;
                case 'Constitutional Generals':
                case 'Thermal State':
                case 'Food & Cravings':
                    categoryMultiplier = scoringConfig_1.CLINICAL_SCORING_CONFIG.physicalGenerals;
                    break;
                case 'Modalities':
                    categoryMultiplier = scoringConfig_1.CLINICAL_SCORING_CONFIG.modalities;
                    break;
                case 'Sleep':
                    categoryMultiplier = scoringConfig_1.CLINICAL_SCORING_CONFIG.physicalGenerals;
                    break;
                default:
                    categoryMultiplier = scoringConfig_1.CLINICAL_SCORING_CONFIG.particulars;
            }
            ...
            for (const rem of rub.relatedRemedies) {
                ...
                contribution += (miasmFit * scoringConfig_1.CLINICAL_SCORING_CONFIG.miasmaticFit) / 100;
                ...
                contribution += (constFit * scoringConfig_1.CLINICAL_SCORING_CONFIG.constitutionalFit) / 100;
        ...
        const rankedRemedies = await Promise.all(Object.values(scores).map(async (r) => {
            // Fetch metadata for remedy
            const meta = repertoryData_1.REMEDIES_METADATA[r.remedyId] || { fullName: r.remedyName, source: 'Plant' };
            const confirmations = repertoryData_1.JETHWANI_REMEDY_CONFIRMATIONS[r.remedyId];
            let miasm = 'Psora';
            if (confirmations?.confirmatory.some(c => c.toLowerCase().includes('sycotic')))
                miasm = 'Sycosis';
            else if (confirmations?.confirmatory.some(c => c.toLowerCase().includes('syphilitic')))
                miasm = 'Syphilis';
            let thermal = 'Ambient';
            if (confirmations?.confirmatory.some(c => c.toLowerCase().includes('chilly') || c.toLowerCase().includes('cold')))
                thermal = 'Chilly';
            else if (confirmations?.confirmatory.some(c => c.toLowerCase().includes('warm') || c.toLowerCase().includes('hot')))
                thermal = 'Warm';
        ...
        if (rankedRemedies.length >= 2) {
            const diffs = await repertoryGraph_1.RepertoryGraph.getDifferentiatingRubrics(rankedRemedies[0].remedyId, rankedRemedies[1].remedyId);
            result.differentiatingRubrics = diffs.slice(0, 3).map(d => d.rubric.rubricId);
        }
        // 6. Gather missing parameters
        if (!hasThermalSelected)
            result.missingDataNeeded.push("Thermal State (Chilly / Warm / Ambient)");
        if (!hasThirstSelected)
            result.missingDataNeeded.push("Thirst Pattern (Thirsty large / Thirstless)");
        if (!hasModalitiesSelected)
            result.missingDataNeeded.push("Symptom Modalities (Aggravations / Ameliorations)");
        if (!hasMensesSelected)
            result.missingDataNeeded.push("Menses details (for female cases)");
        if (!hasSleepSelected)
            result.missingDataNeeded.push("Sleep / Dreams details");
        if (!hasEtiologySelected)
            result.missingDataNeeded.push("Etiology / Causation (Onset triggers)");
        return result;
    }
    /**
     * Performs deep differentiation analysis for top remedy candidates.
     */
    static async differentiateRemedies(topRemedyIds, selectedRubricIds) {
        const differentiations = [];
        const activeRubrics = [];
        for (const rid of selectedRubricIds) {
            const rub = await repertoryDb_1.repertoryRepository.getRubricById(rid);
            if (rub)
                activeRubrics.push(rub);
        }
        for (const remedyId of topRemedyIds) {
            const meta = repertoryData_1.REMEDIES_METADATA[remedyId];
            if (!meta)
                continue;
            const confirmations = repertoryData_1.JETHWANI_REMEDY_CONFIRMATIONS[remedyId] || { confirmatory: [], eliminating: [], differentiating: [] };
            ...
            const missingConfirming = [];
            confirmations.confirmatory.forEach(conf => {
                const isMatched = activeRubrics.some(r => r.title.toLowerCase().includes(conf.toLowerCase()) ||
            ...
            const strongestMatch = activeRubrics.find(r => r.relatedRemedies.some(rem => rem.remedyId.toLowerCase() === remedyId.toLowerCase() && rem.grade >= 3));
...
exports.RepertoryScoring = RepertoryScoring;
