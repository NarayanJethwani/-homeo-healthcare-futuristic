...
exports.runQualityGateChecks = runQualityGateChecks;
 ...
 */
function runQualityGateChecks(entity, allEntities) {
    const issues = [];
    const prohibitedClaimsFound = [];
    // Helper to audit text blocks
    const scanTextForClaims = (text, path) => {
        if (!text)
            return;
        const lower = text.toLowerCase();
        constants_1.PROHIBITED_CLAIMS_PHRASES.forEach(phrase => {
