...
exports.getRelationshipSuggestions = getRelationshipSuggestions;
// Preset mapping dictionary for common clinical conditions to ensure accurate suggestions
const CLINICAL_AFFINITY_DICTIONARY = {
    migraine: {
 ...
 */
function getRelationshipSuggestions(entity, allEntities) {
    const suggestions = [];
    const titleLower = (entity.title.en || "").toLowerCase();
    // Find preset dictionary key
    const matchKey = Object.keys(CLINICAL_AFFINITY_DICTIONARY).find(key => titleLower.includes(key) || key.includes(titleLower));
    const presets = matchKey ? CLINICAL_AFFINITY_DICTIONARY[matchKey] : null;
    for (const item of allEntities) {
        if (item.id === entity.id)
            continue;
        if (entity.relatedEntities.includes(item.id))
