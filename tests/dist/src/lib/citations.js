...
exports.CORE_CITATIONS_DB = void 0;
exports.getAuthorLevel = getAuthorLevel;
exports.getEvidencePanel = getEvidencePanel;
 ...
 */
function getAuthorLevel(authorName) {
    const norm = authorName.toLowerCase().trim();
    if (["kent", "james tyler kent", "j. t. kent", "hering", "constantine hering", "lippe", "adolph lippe", "allen", "h. c. allen", "nash", "e. b. nash", "boericke", "william boericke", "clarke", "john henry clarke", "hahnemann", "samuel hahnemann"].includes(norm)) {
        return 1;
    }
    if (["boger", "cyrus maxwell boger", "c. m. boger", "bönninghausen", "clemens von bönninghausen", "boenninghausen", "farrington", "ernest farrington", "guernsey", "henry n. guernsey", "adolph von lippe"].includes(norm)) {
        return 2;
    }
    if (["tyler", "margaret tyler", "m. l. tyler", "borland", "douglas borland"].includes(norm)) {
        return 3;
    }
    if (["sankaran", "rajan sankaran", "scholten", "jan scholten", "mangialavori", "massimo mangialavori", "sherr", "jeremy sherr"].includes(norm)) {
        return 4;
    }
 ...
 */
function getEvidencePanel(remedyId, itemKey) {
    const canonId = (0, normalizationEngine_1.resolveCanonicalRemedyId)(remedyId);
    const remedy = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === canonId);
    ...
    if (exports.CORE_CITATIONS_DB[canonId] && exports.CORE_CITATIONS_DB[canonId][key]) {
        citations = [...exports.CORE_CITATIONS_DB[canonId][key]];
