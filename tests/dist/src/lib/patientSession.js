...
exports.PATIENT_SESSION_COOKIE = void 0;
exports.createPatientSessionCookie = createPatientSessionCookie;
exports.verifyPatientSessionCookie = verifyPatientSessionCookie;
exports.PATIENT_SESSION_COOKIE = "hh_patient_session";
...
async function createPatientSessionCookie(payload) {
    const encodedPayload = encodeJsonPayload(payload);
    const signature = await sign(encodedPayload);
    return `${encodedPayload}.${signature}`;
}
async function verifyPatientSessionCookie(cookieValue) {
    if (!cookieValue)
        return null;
    ...
    try {
        ...
        if (!payload.uid || payload.role !== "patient" || !payload.exp)
            return null;
