...
exports.CLINICAL_BOARD_QUESTIONS = exports.parseLearningTutorQuery = exports.REMEDY_LEARNING_DB = exports.compareMiasms = exports.compareKingdoms = exports.compareFamilies = exports.compareRemedies = exports.MIASM_PROFILES = exports.KINGDOM_PROFILES = exports.searchRemedies = exports.MASTER_REMEDY_DB = void 0;
const materiaMedicaDb_1 = require("./materiaMedicaDb");
Object.defineProperty(exports, "MASTER_REMEDY_DB", { enumerable: true, get: function () { return materiaMedicaDb_1.MASTER_REMEDY_DB; } });
...
const searchRemedies = (filters) => {
    const results = [];
    let canonicalIdQuery = null;
    if (filters.symptomQuery) {
        const resolved = (0, normalizationEngine_1.resolveCanonicalRemedyId)(filters.symptomQuery);
        if (materiaMedicaDb_1.MASTER_REMEDY_DB.some(r => r.id === resolved)) {
            canonicalIdQuery = resolved;
        }
    }
    if (!canonicalIdQuery && filters.themeQuery) {
        const resolved = (0, normalizationEngine_1.resolveCanonicalRemedyId)(filters.themeQuery);
        if (materiaMedicaDb_1.MASTER_REMEDY_DB.some(r => r.id === resolved)) {
            canonicalIdQuery = resolved;
        }
    }
    for (const remedy of materiaMedicaDb_1.MASTER_REMEDY_DB) {
        let score = 0;
        const reasons = [];
        ...
        if (filters.symptomQuery) {
            ...
            const matchingConditions = remedy.clinicalConditions.filter(c => c.condition.toLowerCase().includes(q.toLowerCase()) ||
        ...
        if (filters.modalities && filters.modalities.length > 0) {
            ...
            const matchedMods = filters.modalities.filter(m => allMods.some(am => am.toLowerCase().includes(m.toLowerCase())));
        ...
        if (filters.fears && filters.fears.length > 0) {
            const matchedFears = filters.fears.filter(f => remedy.mentalPicture.fears.some(rf => rf.toLowerCase().includes(f.toLowerCase())) ||
        ...
        if (filters.dreams && filters.dreams.length > 0) {
            const matchedDreams = filters.dreams.filter(d => remedy.physicalGenerals.dreams.some(rd => rd.toLowerCase().includes(d.toLowerCase())));
        ...
        if (filters.foodDesires && filters.foodDesires.length > 0) {
            const matchedDesires = filters.foodDesires.filter(fd => remedy.physicalGenerals.foodDesires.some(rfd => rfd.toLowerCase().includes(fd.toLowerCase())));
        ...
        if (filters.foodAversions && filters.foodAversions.length > 0) {
            const matchedAversions = filters.foodAversions.filter(fa => remedy.physicalGenerals.foodAversions.some(rfa => rfa.toLowerCase().includes(fa.toLowerCase())));
        ...
        if (filters.organAffinities && filters.organAffinities.length > 0) {
            const matchedOrgans = filters.organAffinities.filter(o => remedy.organAffinities.some(ra => ra.organ.toLowerCase().includes(o.toLowerCase())));
        ...
        if (filters.kingdoms && filters.kingdoms.length > 0) {
            const matchedKingdom = filters.kingdoms.some(k => remedy.identity.kingdom.toLowerCase() === k.toLowerCase());
            if (matchedKingdom) {
                score += 10;
                reasons.push({
                    facet: "Kingdom",
...
exports.searchRemedies = searchRemedies;
// Kingdom Data Templates
exports.KINGDOM_PROFILES = {
    plant: {
        themes: [
...
// COMPARE FUNCTIONS
const compareRemedies = (idA, idB) => {
    const remedyA = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === idA);
    const remedyB = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === idB);
    return { remedyA, remedyB };
};
exports.compareRemedies = compareRemedies;
const compareFamilies = (familyAName, familyBName) => {
    const remediesA = materiaMedicaDb_1.MASTER_REMEDY_DB.filter(r => r.identity.family.toLowerCase().includes(familyAName.toLowerCase()));
    const remediesB = materiaMedicaDb_1.MASTER_REMEDY_DB.filter(r => r.identity.family.toLowerCase().includes(familyBName.toLowerCase()));
    // Average miasm profile
    const avgMiasmA = { psora: 0, sycosis: 0, syphilis: 0, tubercular: 0, cancerinic: 0 };
    const avgMiasmB = { psora: 0, sycosis: 0, syphilis: 0, tubercular: 0, cancerinic: 0 };
...
exports.compareFamilies = compareFamilies;
const compareKingdoms = (kingdomAName, kingdomBName) => {
    const normalizedK_A = kingdomAName.toLowerCase();
    const normalizedK_B = kingdomBName.toLowerCase();
    const remediesA = materiaMedicaDb_1.MASTER_REMEDY_DB.filter(r => r.identity.kingdom.toLowerCase() === normalizedK_A).map(r => r.identity.name);
    const remediesB = materiaMedicaDb_1.MASTER_REMEDY_DB.filter(r => r.identity.kingdom.toLowerCase() === normalizedK_B).map(r => r.identity.name);
    const profileA = exports.KINGDOM_PROFILES[normalizedK_A] || { themes: [], somatic: [], miasms: [], speed: "Unknown" };
    const profileB = exports.KINGDOM_PROFILES[normalizedK_B] || { themes: [], somatic: [], miasms: [], speed: "Unknown" };
    return {
...
exports.compareKingdoms = compareKingdoms;
const compareMiasms = (miasmAName, miasmBName) => {
    const normalizedM_A = miasmAName.toLowerCase();
    const normalizedM_B = miasmBName.toLowerCase();
    const profileA = exports.MIASM_PROFILES[normalizedM_A] || { core: "", dermal: "", mental: "", modalities: [] };
    const profileB = exports.MIASM_PROFILES[normalizedM_B] || { core: "", dermal: "", mental: "", modalities: [] };
    // Find representative remedies (ones where the miasm is >= 40%)
    const repsA = materiaMedicaDb_1.MASTER_REMEDY_DB.filter(r => {
        const miasmVal = r.miasmaticAnalysis[normalizedM_A];
        return typeof miasmVal === "number" && miasmVal >= 40;
    }).map(r => `${r.identity.name} (${r.miasmaticAnalysis[normalizedM_A]}%)`);
    const repsB = materiaMedicaDb_1.MASTER_REMEDY_DB.filter(r => {
        const miasmVal = r.miasmaticAnalysis[normalizedM_B];
        return typeof miasmVal === "number" && miasmVal >= 40;
    }).map(r => `${r.identity.name} (${r.miasmaticAnalysis[normalizedM_B]}%)`);
    return {
        miasmA: miasmAName,
        miasmB: miasmBName,
...
exports.compareMiasms = compareMiasms;
// ==========================================
...
const generateLearningDb = () => {
    const db = {};
    for (const rem of materiaMedicaDb_1.MASTER_REMEDY_DB) {
        const id = rem.id;
        ...
        quizzes.forEach((q, qidx) => {
            let correctAns = "";
            if (qidx === 0)
                correctAns = rem.modalities.worseFrom[0] || "Stuffy closed rooms";
            else if (qidx === 1)
                correctAns = rem.miasmaticAnalysis.dominantMiasm;
            else
                correctAns = `${rem.organAffinities[0]?.organ || "Skin"} (Rating: ${rem.organAffinities[0]?.rating || 10}/10)`;
            // Set correct index
...
exports.REMEDY_LEARNING_DB = generateLearningDb();
const parseLearningTutorQuery = (queryText) => {
    const norm = queryText.toLowerCase().trim();
    const graph = (0, knowledgeGraph_1.getKnowledgeGraph)();
    ...
    const db = exports.REMEDY_LEARNING_DB[remedyId] || exports.REMEDY_LEARNING_DB.rem_sulphur;
    // 1. Quiz Me On...
    ...
    if (norm.includes('compare') || norm.includes('versus') || norm.includes('vs')) {
        ...
        const db2 = exports.REMEDY_LEARNING_DB[secondId] || exports.REMEDY_LEARNING_DB.rem_lycopodium;
        const rem1 = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === remedyId);
        const rem2 = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === secondId);
        return {
    ...
    const rem = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === remedyId);
    return {
...
exports.parseLearningTutorQuery = parseLearningTutorQuery;
exports.CLINICAL_BOARD_QUESTIONS = [
    {
