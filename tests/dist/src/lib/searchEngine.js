...
const parseNaturalLanguageQuery = (queryText) => {
    ...
    const resolvedDirect = (0, normalizationEngine_1.resolveCanonicalRemedyId)(normalizedText);
    ...
    queryWords.forEach(word => {
        if (word.length >= 3) {
            const resolvedWord = (0, normalizationEngine_1.resolveCanonicalRemedyId)(word);
    ...
    remedies.forEach(remedy => {
        ...
        matchedEntities.forEach(entity => {
            ...
            const directEdge = graph.edges.find(e => (e.source === remedy.id && e.target === entity.id) ||
