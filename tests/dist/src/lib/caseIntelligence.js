...
exports.CONFIRMATORY_QUESTIONS_DB = exports.CONSTITUTIONAL_ARCHETYPES = void 0;
exports.getGenomeSimilarity = getGenomeSimilarity;
exports.getClosestRemedies = getClosestRemedies;
exports.getRemedyDifferentiation = getRemedyDifferentiation;
exports.recognizeCaseArchetype = recognizeCaseArchetype;
exports.getConfirmatoryQuestions = getConfirmatoryQuestions;
exports.getSimulatedCase = getSimulatedCase;
 ...
 */
function getGenomeSimilarity(idA, idB) {
    const canonA = (0, normalizationEngine_1.resolveCanonicalRemedyId)(idA);
    const canonB = (0, normalizationEngine_1.resolveCanonicalRemedyId)(idB);
    const remA = remedyGenomeSchema_1.GENOME_REMEDY_DB.find(r => r.id === canonA);
    const remB = remedyGenomeSchema_1.GENOME_REMEDY_DB.find(r => r.id === canonB);
    if (!remA || !remB)
        return 50;
    let sumSq = 0;
    let count = 0;
 ...
 */
function getClosestRemedies(remedyId, limit = 20) {
    const canonId = (0, normalizationEngine_1.resolveCanonicalRemedyId)(remedyId);
    const target = remedyGenomeSchema_1.GENOME_REMEDY_DB.find(r => r.id === canonId);
    if (!target)
        return [];
    const distances = remedyGenomeSchema_1.GENOME_REMEDY_DB
 ...
 */
function getRemedyDifferentiation(idA, idB) {
    const canonA = (0, normalizationEngine_1.resolveCanonicalRemedyId)(idA);
    const canonB = (0, normalizationEngine_1.resolveCanonicalRemedyId)(idB);
    const remA = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === canonA);
    const remB = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === canonB);
    const genA = remedyGenomeSchema_1.GENOME_REMEDY_DB.find(r => r.id === canonA);
    const genB = remedyGenomeSchema_1.GENOME_REMEDY_DB.find(r => r.id === canonB);
    ...
    const getCompareText = (valA, valB, label) => {
...
exports.CONSTITUTIONAL_ARCHETYPES = [
    {
...
function recognizeCaseArchetype(symptomText) {
    const text = symptomText.toLowerCase();
    const results = exports.CONSTITUTIONAL_ARCHETYPES.map(arch => {
        let matchCount = 0;
...
exports.CONFIRMATORY_QUESTIONS_DB = {
    "rem_sulphur": [
...
function getConfirmatoryQuestions(remedyId) {
    const canon = (0, normalizationEngine_1.resolveCanonicalRemedyId)(remedyId);
    if (exports.CONFIRMATORY_QUESTIONS_DB[canon]) {
        return exports.CONFIRMATORY_QUESTIONS_DB[canon];
    }
    const rem = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === canon);
    if (rem) {
...
function getSimulatedCase(idx) {
    ...
    const rem = materiaMedicaDb_1.MASTER_REMEDY_DB.find(r => r.id === correctRemedyId) || materiaMedicaDb_1.MASTER_REMEDY_DB[0];
    const potencies = ["30C", "200C", "1M", "10M"];
    const correctPotency = potencies[(hash1 + hash2) % potencies.length];
    const difficultyLevel = index % 3 === 0 ? "Beginner" : index % 3 === 1 ? "Intermediate" : "Advanced";
