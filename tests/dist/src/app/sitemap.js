...
const collectionsRegistry_1 = require("@/features/knowledge/collections/collectionsRegistry");
const comparisonRegistry_1 = require("@/features/knowledge/comparisons/comparisonRegistry");
async function sitemap() {
    ...
    try {
        const entities = (0, knowledge_1.getAllKnowledgeEntities)().filter((e) => e.editorialStatus === "published");
        for (const entity of entities) {
            const entityRoute = (0, knowledge_1.getEntityUrl)(entity.entityType, entity.slug);
            sitemapItems.push({
    ...
    try {
        for (const collection of collectionsRegistry_1.CURATED_COLLECTIONS) {
            sitemapItems.push({
    ...
    try {
        for (const comp of comparisonRegistry_1.COMPARISONS) {
            sitemapItems.push({
