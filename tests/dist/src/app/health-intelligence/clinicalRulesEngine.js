...
exports.analyzeDigitalTwin = analyzeDigitalTwin;
exports.getRelatedContent = getRelatedContent;
function analyzeDigitalTwin(twin) {
    const activeFlags = [];
    const priorityGoals = [];
    // Baselines
    const systemScores = {
    ...
    completedKeys.forEach(key => {
        ...
        if (["hypertension", "heart_disease", "stroke_risk", "lipid_health", "cardio_age"].includes(key)) {
            ...
            if (key === "hypertension")
                organLoad.arteries = Math.max(organLoad.arteries, burden);
            if (key === "heart_disease")
                organLoad.heart = Math.max(organLoad.heart, burden);
        ...
        if (["ibs_assessment", "gut_health", "gerd_evaluation", "liver_health", "fatty_liver_risk"].includes(key)) {
            ...
            if (key === "ibs_assessment" || key === "gut_health")
                organLoad.gut = Math.max(organLoad.gut, burden);
            if (key === "liver_health" || key === "fatty_liver_risk")
                organLoad.liver = Math.max(organLoad.liver, burden);
