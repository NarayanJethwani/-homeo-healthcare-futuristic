    ...
    try {
        ...
        if (action === "save") {
            if (!rubricData || !rubricData.name) {
                return server_1.NextResponse.json({ success: false, message: "Rubric name is required." }, { status: 400 });
            }
        ...
        if (action === "merge") {
            ...
            if (!targetName || !sourceIds || sourceIds.length === 0) {
                return server_1.NextResponse.json({ success: false, message: "Target name and source IDs are required." }, { status: 400 });
            }
            const sourceRubrics = [];
            ...
            if (sourceRubrics.length === 0) {
                return server_1.NextResponse.json({ success: false, message: "No source rubrics found." }, { status: 404 });
            }
        ...
        if (action === "clone") {
            ...
            if (!sourceId || !newName) {
                return server_1.NextResponse.json({ success: false, message: "Source ID and new name are required." }, { status: 400 });
            }
            const docSnap = await rubricsRef.doc(sourceId).get();
            if (!docSnap.exists) {
                return server_1.NextResponse.json({ success: false, message: "Source rubric not found." }, { status: 404 });
            }
            const source = docSnap.data();
            const id = `custom_${generateSlug(newName)}_${Date.now()}`;
            const slug = generateSlug(newName);
            const words = newName.toLowerCase().split(/[\s,\.\-_]+/);
            const rubric = {
                ...source,
                id,
                name: newName,
        ...
        return server_1.NextResponse.json({ success: false, message: "Invalid action." }, { status: 400 });
    }
    catch (error) {
