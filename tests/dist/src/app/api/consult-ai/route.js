...
function checkMedicalSafety(query) {
    const q = query.toLowerCase();
    // Crisis prevention
...
async function POST(request) {
    try {
        ...
        const result = await aiRouter_1.aiRouterService.consultAI(query, systemInstruction, {
            score,
            answers,
            logs,
            mode,
        ...
        return server_1.NextResponse.json(result, { headers: CORS_HEADERS });
    }
    catch (error) {
