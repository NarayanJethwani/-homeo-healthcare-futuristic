...
async function POST(request) {
    try {
        ...
        if (!patientId || !aiReport) {
            return server_1.NextResponse.json({ success: false, message: "Missing patientId or aiReport parameter." }, { status: 400 });
        }
        ...
        try {
            ...
            if (trimmedReport.startsWith("{")) {
                ...
                if (data.remedy_battlefield && data.remedy_battlefield.length > 0) {
                    const bfText = data.remedy_battlefield.map((b) => `* ${b.remedy}:\n` +
                        `  Match %: ${b.match_pct || 0}% | Confidence: ${b.confidence_pct || 0}%\n` +
                        `  Matches: Mental (${b.mental_match || 0}%), General (${b.general_match || 0}%), Modality (${b.modality_match || 0}%), Constitutional (${b.constitution_match || 0}%)`).join("\n");
                    cmdCenter.push(`REMEDY BATTLEFIELD COMPARISON:\n${bfText}`);
                }
                if (data.remedy_confirmation && data.remedy_confirmation.length > 0) {
                    const confirmText = data.remedy_confirmation.map((c) => `* ${c.remedy}:\n` +
                        `  - What would confirm: ${(c.confirm_questions || []).join(" | ")}\n` +
                        `  - What would rule out: ${(c.rule_out_questions || []).join(" | ")}`).join("\n");
                    cmdCenter.push(`REMEDY CONFIRMATION ENGINE:\n${confirmText}`);
                }
                if (data.contradictions && data.contradictions.length > 0) {
                    const contraText = data.contradictions.map((c) => `- Symptom "${c.symptom}" contradicts ${c.remedy}: ${c.reason}`).join("\n");
                    cmdCenter.push(`CONTRADICTION DETECTOR:\n${contraText}`);
                }
                if (data.missing_information && data.missing_information.length > 0) {
                    const missingText = data.missing_information.map((m) => `- ${m}`).join("\n");
                ...
                if (data.explainability_layer && data.explainability_layer.length > 0) {
                    const expText = data.explainability_layer.map((e) => `* Conclusion: ${e.conclusion}\n` +
                        `  Rationale: ${e.rationale}\n` +
                        `  Influencing Rubrics: ${(e.rubrics || []).join(", ") || "None"}\n` +
                        `  Constitutional Factors: ${(e.constitutional_factors || []).join(", ") || "None"}`).join("\n");
                ...
                if (data.materia_medica_analysis) {
                    ...
                    if (mma.differential_comparison && mma.differential_comparison.length > 0) {
                        const diffCards = mma.differential_comparison.map((dc) => `* Remedy: ${dc.remedy}\n` +
                            ...
                            `  - Miasmatic Differences: ${dc.miasmatic_differences || "N/A"}`).join("\n\n");
                    ...
                    if (mma.confirmatory_questions && mma.confirmatory_questions.length > 0) {
                        const cqText = mma.confirmatory_questions.map((q) => `- ${q}`).join("\n");
                        mmaSections.push(`MATERIA MEDICA CONFIRMATORY QUESTIONS:\n${cqText}`);
                    }
                    if (mma.contradictions && mma.contradictions.length > 0) {
                        const contraText = mma.contradictions.map((c) => `- Remedy: ${c.remedy} | Contradicting Symptom: "${c.symptom}" | Reason: ${c.reason}`).join("\n");
                ...
                if (data.followup_progress && data.followup_progress.length > 0) {
                    const progressText = data.followup_progress.map((p) => `- ${p.date || p.visit || "Follow-up"}: Severity Index ${p.severity || 0}%`).join("\n");
                ...
                if (data.followup_questions && data.followup_questions.length > 0) {
                    const qs = data.followup_questions.map((q) => `- ${q}`).join("\n");
        ...
        if (sheetId && sheetId !== "mock-sheet-id" && sheetId !== "mock-sheet") {
            try {
                await (0, googleDrive_1.appendAiReportToClinicalSheet)(sheetId, formattedReport);
            }
            catch (sheetErr) {
