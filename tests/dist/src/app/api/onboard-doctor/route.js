...
async function POST(request) {
    try {
        ...
        const { name, email, phone = "", speciality = "General Homeopathy", plan = "monthly", adminUid = "", } = body;
        if (!name || !email) {
            return server_1.NextResponse.json({ success: false, message: "Doctor name and email are required." }, { status: 400 });
        }
        ...
        if (isFirebaseConfigured) {
            try {
                ...
                try {
                    const existingUser = await (0, firebaseAdmin_1.getAdminAuth)().getUserByEmail(email);
                    ...
                    const userRecord = await (0, firebaseAdmin_1.getAdminAuth)().createUser({
                        email,
                        displayName: name,
                    ...
                    try {
                        const resetLink = await (0, firebaseAdmin_1.getAdminAuth)().generatePasswordResetLink(email);
                ...
                if (!authErr.message?.includes("already exists")) {
                    return server_1.NextResponse.json({ success: false, message: "Firebase Auth error: " + authErr.message }, { status: 500 });
                }
            }
        }
        // ── 2. Provision Google Drive folder + Master Sheet ───────────────────────
        const workspace = await (0, googleDrive_1.createDoctorWorkspace)(name, email);
        ...
        if (isFirebaseConfigured) {
            // Write to users/{uid}
            await (0, firebaseAdmin_1.getAdminDb)().collection("users").doc(uid).set(userProfile, { merge: true });
            // Write to doctors/{uid} (workspace metadata for Firestore rules)
            await (0, firebaseAdmin_1.getAdminDb)().collection("doctors").doc(uid).set({
                uid,
                name,
                email,
                driveFolderId: workspace.driveFolderId,
