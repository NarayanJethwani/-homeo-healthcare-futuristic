...
exports.compileLocalSynthesisResponse = compileLocalSynthesisResponse;
exports.getPromptsForTask = getPromptsForTask;
exports.POST = POST;
...
function compileLocalSynthesisResponse(taskType, body) {
    ...
    if (taskType === "clinical_reasoning") {
        ...
        const thermals = rubrics.some((r) => r.name.toLowerCase().includes("warm") || r.name.toLowerCase().includes("heat")) ? "Warm-blooded" : "Chilly";
        const hasSkin = rubrics.some((r) => r.chapter.toLowerCase().includes("skin") || r.name.toLowerCase().includes("skin") || r.name.toLowerCase().includes("itching"));
        const hasDigestive = rubrics.some((r) => r.chapter.toLowerCase().includes("stomach") || r.chapter.toLowerCase().includes("abdomen") || r.name.toLowerCase().includes("flatulence") || r.name.toLowerCase().includes("bloat"));
        const hasMind = rubrics.some((r) => r.chapter.toLowerCase().includes("mind") || r.name.toLowerCase().includes("anxiety") || r.name.toLowerCase().includes("fear"));
        const systems = [];
        if (hasSkin)
            systems.push("Integumentary System");
        if (hasDigestive)
            systems.push("Gastrointestinal System");
        if (hasMind)
            systems.push("Nervous System (Autonomic)");
        if (systems.length === 0)
            systems.push("Constitutional / Systemic Regulation");
        const patterns = [];
        if (hasSkin)
            patterns.push("Dermal Inflammatory Reactivity with Suppression Risk");
        if (hasDigestive)
            patterns.push("Functional Gastric Dysmotility & Visceral Hypersensitivity");
        if (hasMind)
            patterns.push("Sympathetic Nervous Overdrive with Health Anticipation");
        if (patterns.length === 0)
            patterns.push("Functional Somatic Energy Stagnation");
        const jethwaniOrClinicalRubrics = rubrics.filter((r) => {
        ...
        if (jethwaniOrClinicalRubrics.length > 0) {
            const adrenalSymptom = rubrics.some((r) => {
            ...
            const suppressionSymptom = rubrics.some((r) => {
                const name = (r.name || "").toLowerCase();
        ...
        return {
            ...
            analysis: {
                clinical_reasoning_v2: {
                    ...
                    remedy_justification: `Prescription of ${mainRemedy} is justified by its high coverage of the patient's symptoms (${rubrics.slice(0, 3).map((r) => r.name).join(", ")}) and matching thermal affinity.`,
...
In a case of chronic migraine, standard pathology focuses on vascular dilation. A homeopath, however, seeks the peculiar characteristics: for example, if the migraine is relieved only by binding the head tightly (Argentum Nitricum) or is accompanied by a sensation of coldness in the brain (Phosphorus). These peculiar keynotes guide us to the curative simillimum.`;
    ...
    const thermals = rubrics.some((r) => r.name.toLowerCase().includes("warm") || r.name.toLowerCase().includes("heat") || r.name.toLowerCase().includes("hot")) ? "Warm-blooded" : "Chilly";
    const hasSkin = rubrics.some((r) => r.chapter.toLowerCase().includes("skin") || r.name.toLowerCase().includes("skin") || r.name.toLowerCase().includes("itching") || r.name.toLowerCase().includes("eruption"));
    const hasDigestive = rubrics.some((r) => r.chapter.toLowerCase().includes("stomach") || r.chapter.toLowerCase().includes("abdomen") || r.name.toLowerCase().includes("flatulence") || r.name.toLowerCase().includes("bloat") || r.name.toLowerCase().includes("constipation"));
    const hasMind = rubrics.some((r) => r.chapter.toLowerCase().includes("mind") || r.name.toLowerCase().includes("anxiety") || r.name.toLowerCase().includes("fear") || r.name.toLowerCase().includes("irritab"));
    let psoraScore = 45;
    let sycosisScore = 20;
    ...
    jethwaniOrClinicalRubrics.forEach((r) => {
        const name = (r.name || "").toLowerCase();
    ...
    return {
        constitutional_profile: {
            ...
            suppression_history: rubrics.some((r) => {
                const name = (r.name || "").toLowerCase();
                return name.includes("steroid") || name.includes("suppress") || name.includes("cortisone");
            })
        ...
        explainability_layer: [
            {
                ...
                rubrics: rubrics.slice(0, 3).map((r) => r.name),
        ...
        miasmatic_analysis: {
            ...
            description: jethwaniOrClinicalRubrics.length > 0
                ? `Dominant ${dominantMiasm} miasm with specialized indications from Jethwani/Clinical repertories. Highlighted by ${rubrics.some((r) => {
                    const name = (r.name || "").toLowerCase();
                    return name.includes("adrenal") || name.includes("burnout") || name.includes("fatigue") || name.includes("exhaustion") || name.includes("post-viral");
        ...
        differential_matrix: [
            {
                remedy: mainRemedy,
                matches: rubrics.slice(0, 3).map((r) => r.name),
            ...
            {
                remedy: secondaryRemedy,
                matches: rubrics.slice(1, 4).map((r) => r.name),
...
function getPromptsForTask(taskType, body) {
    const patientName = body?.patientInfo?.name || body?.patientName || "Patient";
    const age = body?.patientInfo?.age || body?.age || "35";
    const gender = body?.patientInfo?.gender || body?.gender || "Male";
  ...
  "clinical_conference": {
    ...
    "consensus": {
      ...
      "report": "Comprehensive Final Case Conference Report summarizing the panel's findings and plan."
        ...
        const rubricsText = (body?.rubrics || []).map((r) => `- ${r.chapter}: ${r.name} (Source: ${r.source || "Kent"}, Grade: ${r.grade || 1})`).join("\n");
  ...
  "clinical_hypothesis_engine": [
    {
      ...
      "suggested_investigations": ["string suggested investigations or tests"]
        ...
        const rubricsPrompt = (body?.rubrics || []).map((r) => `- [${r.chapter}] ${r.name} (Source: ${r.source || "Kent"}, Intensity/Severity Grade: ${r.grade})`).join("\n");
        const repertorizationResults = body?.repertorizationResults || [];
...
${repertorizationResults.map((res) => `- ${res.remedyName}: Coverage = ${res.coverage}, Sum of Grades = ${res.score}`).join("\n")}
...
Please provide your tutor response. Ensure the JSON is completely filled.`;
    ...
    const matchedGraphRemedies = (body?.repertorizationResults || []).map((res) => {
...
async function POST(request) {
    ...
    try {
        ...
        let routerConfig = {
            ...
            ragPriority: ["jethwani", "kent", "boericke", "allen", "phatak", "clarke", "outcome", "graph"],
            conferenceMode: "weighted",
            conferenceWeights: {
                gemini: 40,
                deepseek: 35,
                qwen: 25
            },
            expertVetoModel: "gemini"
        };
        if (body.routerConfig) {
            ...
            try {
                const configDocRef = (0, firestore_1.doc)(firebase_1.db, "settings", "ai_router_config");
                const docSnap = await withTimeout((0, firestore_1.getDoc)(configDocRef), 3000);
                if (docSnap.exists()) {
        ...
        if (!apiKey) {
            ...
            const logsCollectionRef = (0, firestore_1.collection)(firebase_1.db, "ai_telemetry_logs");
            (0, firestore_1.addDoc)(logsCollectionRef, logEntryWithId).catch((logErr) => {
