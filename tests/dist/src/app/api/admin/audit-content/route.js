...
async function POST(request) {
    ...
    try {
        ...
        if (tags && !Array.isArray(tags)) {
            return server_1.NextResponse.json({ success: false, error: "Invalid payload: 'tags' must be an array of strings." }, { status: 400, headers: CORS_HEADERS });
        }
...
Do not include any markdown formatting wrappers (like \`\`\`json) or conversational text outside the JSON.`;
        ...
        const aiResponse = await aiRouter_1.aiRouterService.consultAI(userPrompt, systemInstruction, {
            mode: "doctor",
            lang: "en"
        });
        if (!aiResponse.success) {
            return server_1.NextResponse.json({ success: false, error: "AI Router failed to audit the content. Provider may be offline." }, { status: 502, headers: CORS_HEADERS });
        }
        let auditResults;
