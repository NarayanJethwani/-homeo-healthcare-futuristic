...
import {
  M15_ENTITY_PROFILES,
  M15_OFFLINE_EVALUATION_CASES,
  buildKEP6KeyRemediesWave7Package,
  computeM15EvaluationMetrics,
  generateM15AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6KeyRemediesWave7Package";

export function runKnowledgeKEP6KeyRemediesWave7Test(): void {
  ...
  assert.strictEqual(M15_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M15_ENTITY_PROFILES) {
    const cases = M15_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M15_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM15EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M15_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM15EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M15_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM15EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M15 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6KeyRemediesWave7Test();
