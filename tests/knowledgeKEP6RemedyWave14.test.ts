...
import {
  M27_ENTITY_PROFILES,
  M27_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave14Package,
  computeM27EvaluationMetrics,
  generateM27AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave14Package";

export function runKnowledgeKEP6RemedyWave14Test(): void {
  ...
  assert.strictEqual(M27_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M27_ENTITY_PROFILES) {
    const cases = M27_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M27_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM27EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M27_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM27EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M27_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM27EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M27 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave14Test();
