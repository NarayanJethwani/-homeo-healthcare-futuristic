...
import {
  M22_ENTITY_PROFILES,
  M22_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave9Package,
  computeM22EvaluationMetrics,
  generateM22AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave9Package";

export function runKnowledgeKEP6RemedyWave9Test(): void {
  ...
  assert.strictEqual(M22_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M22_ENTITY_PROFILES) {
    const cases = M22_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M22_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM22EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M22_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM22EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M22_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM22EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M22 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave9Test();
