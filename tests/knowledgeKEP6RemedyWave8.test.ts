...
import {
  M21_ENTITY_PROFILES,
  M21_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave8Package,
  computeM21EvaluationMetrics,
  generateM21AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave8Package";

export function runKnowledgeKEP6RemedyWave8Test(): void {
  ...
  assert.strictEqual(M21_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M21_ENTITY_PROFILES) {
    const cases = M21_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M21_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM21EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M21_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM21EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M21_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM21EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M21 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave8Test();
