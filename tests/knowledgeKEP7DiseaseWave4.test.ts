...
import {
  M19_ENTITY_PROFILES,
  M19_OFFLINE_EVALUATION_CASES,
  buildKEP7DiseaseWave4Package,
  computeM19EvaluationMetrics,
  generateM19AuthorizationReport,
} from "../src/features/knowledge/expansion/kep7DiseaseWave4Package";

export function runKnowledgeKEP7DiseaseWave4Test(): void {
  ...
  assert.strictEqual(M19_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M19_ENTITY_PROFILES) {
    const cases = M19_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M19_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM19EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M19_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM19EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M19_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM19EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("✅ Knowledge KEP-7 Disease Wave 4 test suite passed successfully.");
}

if (require.main === module) {
  runKnowledgeKEP7DiseaseWave4Test();
}
