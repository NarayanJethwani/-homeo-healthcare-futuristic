...
import {
  M14_ENTITY_PROFILES,
  M14_OFFLINE_EVALUATION_CASES,
  buildKEP6KeyRemediesWave6Package,
  computeM14EvaluationMetrics,
  generateM14AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6KeyRemediesWave6Package";

export function runKnowledgeKEP6KeyRemediesWave6Test(): void {
  ...
  assert.strictEqual(M14_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M14_ENTITY_PROFILES) {
    const cases = M14_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M14_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM14EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M14_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM14EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M14_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM14EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M14 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6KeyRemediesWave6Test();
