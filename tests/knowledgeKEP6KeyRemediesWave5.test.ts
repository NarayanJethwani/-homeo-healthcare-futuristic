...
import {
  M13_ENTITY_PROFILES,
  M13_OFFLINE_EVALUATION_CASES,
  buildKEP6KeyRemediesWave5Package,
  computeM13EvaluationMetrics,
  generateM13AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6KeyRemediesWave5Package";

export function runKnowledgeKEP6KeyRemediesWave5Test(): void {
  ...
  assert.strictEqual(M13_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M13_ENTITY_PROFILES) {
    const cases = M13_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M13_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM13EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M13_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM13EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M13_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM13EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M13 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6KeyRemediesWave5Test();
