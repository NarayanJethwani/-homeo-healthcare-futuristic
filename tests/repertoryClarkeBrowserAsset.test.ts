...
assert.ok(clarke.every((rubric) => rubric.scoringMode === "occurrence"));
assert.ok(clarke.every((rubric) => rubric.occurrenceScoringEnabled === true));
assert.ok(clarke.every((rubric) => Object.values(rubric.remedies).every((weight) => weight === 1)));
assert.ok(clarke.filter((rubric) => Object.keys(rubric.remedies).length > 0).length >= 4_000);
assert.ok(clarke.reduce((sum, rubric) => sum + Object.keys(rubric.remedies).length, 0) >= 15_000);
...
assert.deepStrictEqual(clarke.find((rubric) => rubric.name === "Acromegaly")?.remedies, { Thyr: 1 });
assert.equal(clarke.find((rubric) => rubric.name === "Actinomycosis")?.remedies["Kali-i"], 1);
assert.equal(clarke.find((rubric) => rubric.name === "Actinomycosis")?.remedies["Nit-ac"], 1);
