...
import {
  M24_ENTITY_PROFILES,
  M24_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave11Package,
  computeM24EvaluationMetrics,
  generateM24AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave11Package";

export function runKnowledgeKEP6RemedyWave11Test(): void {
  ...
  assert.strictEqual(M24_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M24_ENTITY_PROFILES) {
    const cases = M24_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M24_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM24EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M24_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM24EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M24_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM24EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M24 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave11Test();
