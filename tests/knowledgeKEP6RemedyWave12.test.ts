...
import {
  M25_ENTITY_PROFILES,
  M25_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave12Package,
  computeM25EvaluationMetrics,
  generateM25AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave12Package";

export function runKnowledgeKEP6RemedyWave12Test(): void {
  ...
  assert.strictEqual(M25_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M25_ENTITY_PROFILES) {
    const cases = M25_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M25_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM25EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M25_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM25EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M25_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM25EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M25 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave12Test();
