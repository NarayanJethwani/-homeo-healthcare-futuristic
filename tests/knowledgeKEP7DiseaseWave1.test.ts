...
import {
  M16_ENTITY_PROFILES,
  M16_OFFLINE_EVALUATION_CASES,
  buildKEP7DiseaseWave1Package,
  computeM16EvaluationMetrics,
  generateM16AuthorizationReport,
} from "../src/features/knowledge/expansion/kep7DiseaseWave1Package";

export function runKnowledgeKEP7DiseaseWave1Test(): void {
  ...
  assert.strictEqual(M16_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M16_ENTITY_PROFILES) {
    const cases = M16_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M16_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM16EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M16_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM16EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M16_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM16EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M16 verified: 10 source-bound diseases, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP7DiseaseWave1Test();
