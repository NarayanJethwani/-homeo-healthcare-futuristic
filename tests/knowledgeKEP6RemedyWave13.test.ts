...
import {
  M26_ENTITY_PROFILES,
  M26_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave13Package,
  computeM26EvaluationMetrics,
  generateM26AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave13Package";

export function runKnowledgeKEP6RemedyWave13Test(): void {
  ...
  assert.strictEqual(M26_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M26_ENTITY_PROFILES) {
    const cases = M26_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M26_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM26EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M26_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM26EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M26_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM26EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M26 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave13Test();
