...
import {
  M23_ENTITY_PROFILES,
  M23_OFFLINE_EVALUATION_CASES,
  buildKEP6RemedyWave10Package,
  computeM23EvaluationMetrics,
  generateM23AuthorizationReport,
} from "../src/features/knowledge/expansion/kep6RemedyWave10Package";

export function runKnowledgeKEP6RemedyWave10Test(): void {
  ...
  assert.strictEqual(M23_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M23_ENTITY_PROFILES) {
    const cases = M23_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M23_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM23EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M23_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM23EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M23_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM23EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("M23 verified: 10 source-bound remedies, 100 computed cases with 20 emergency controls, 50 draft-only proposals, RAG off, publication frozen.");
}

if (require.main === module) runKnowledgeKEP6RemedyWave10Test();
