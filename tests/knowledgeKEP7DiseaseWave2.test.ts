...
import {
  M17_ENTITY_PROFILES,
  M17_OFFLINE_EVALUATION_CASES,
  buildKEP7DiseaseWave2Package,
  computeM17EvaluationMetrics,
  generateM17AuthorizationReport,
} from "../src/features/knowledge/expansion/kep7DiseaseWave2Package";

export function runKnowledgeKEP7DiseaseWave2Test(): void {
  ...
  assert.strictEqual(M17_OFFLINE_EVALUATION_CASES.length, 100);
  for (const { entity } of M17_ENTITY_PROFILES) {
    const cases = M17_OFFLINE_EVALUATION_CASES.filter((item) => item.entityId === entity.id);
  ...
  const missedEmergency = M17_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM17EvaluationMetrics(missedEmergency).emergencyEscalationFailureCount, 1);

  const unsupported = M17_OFFLINE_EVALUATION_CASES.map((item) => ({ ...item }));
  ...
  assert.strictEqual(computeM17EvaluationMetrics(unsupported).unsupportedClaimFailureCount, 1);

  const stale = M17_OFFLINE_EVALUATION_CASES.map((item) => ({
    ...item,
  ...
  assert.ok(computeM17EvaluationMetrics(stale).staleRevisionLeakageCount >= 1);
  ...
  console.log("✅ Knowledge KEP-7 Disease Wave 2 test suite passed successfully.");
}

if (require.main === module) {
  runKnowledgeKEP7DiseaseWave2Test();
}
