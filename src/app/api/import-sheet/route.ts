...
function parseSheetDataRows(rows: any[][]): any[] {
  ...
  const getRowValue = (keyPattern: string) => {
...
export async function POST(request: Request) {
  try {
    ...
    if (isGoogleDriveFolder(urlOrId)) {
      const folderId = extractFolderId(urlOrId);
      if (!folderId) {
        return NextResponse.json({ success: false, message: "Invalid Folder ID." }, { status: 400 });
      }
    ...
    if (!spreadsheetId) {
      return NextResponse.json({ success: false, message: "Invalid Spreadsheet ID." }, { status: 400 });
    }

    console.log("Fetching patient sheet from Google Drive ID:", spreadsheetId);
    const rows = await getPatientRowsFromSheet(spreadsheetId);
