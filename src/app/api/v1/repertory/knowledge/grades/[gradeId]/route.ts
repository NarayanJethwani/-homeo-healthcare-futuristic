...
import { RubricRemedyGradeId } from "@/features/repertory/types/remedyTypes";
import { z } from "zod";
...
const ParamsSchema = z.object({
  gradeId: z.string().min(1).max(150).transform(v => v as RubricRemedyGradeId)
});
...
export async function GET(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          error: { code: "INVALID_INPUT", message: "Invalid grade identifier." }
        },
        { status: 400 }
      );
    }
    ...
    const { gradeId } = validatedParams.data;

    const service = getRemedyGradeAccessService();
    const result = await service.getGradeById(auth.context, gradeId);

    if (!result) {
      return NextResponse.json(
        {
          metadata: { requestId, generatedAt: new Date().toISOString(), durationMs: Date.now() - startTime },
          error: { code: "NOT_FOUND", message: "Rubric-remedy grade not found." }
        },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        error: {
          ...
          message: error.message || "Failed to retrieve grade details."
        }
      },
