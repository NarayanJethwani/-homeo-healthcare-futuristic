...
const requestSchema = z.object({
  ...
  reportType: z.enum(["lab", "prescription", "imaging"])
});
...
function localFallbackParser(fileName: string, reportType: "lab" | "prescription" | "imaging"): any {
  ...
  let sourceEvidence: Record<string, any> = {
...
Analyze the document carefully. Be very objective and strict. Assign 'Low' confidence to any fields that are inferred or partially mentioned.`;
    ...
    const timeoutPromise = new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error("Gemini OCR extraction request timed out after 30 seconds.")), 30000)
    );
    ...
    const sanitizedResult = {
      ...
      miasm: Array.isArray(rawData.miasm) ? rawData.miasm.map((m: string) => sanitizeString(m)) : [],
    ...
    const fallbackResult = localFallbackParser(sanitizedFileName, payloadReportType as any);
