...
interface EtiologicalFactorsSectionProps {
  etiologicalFactors: string[];
  onChange: (updated: string[]) => void;
}

export function EtiologicalFactorsSection({
  etiologicalFactors,
  onChange
}: EtiologicalFactorsSectionProps) {
  const [inputValue, setInputValue] = useState("");

  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    const val = inputValue.trim();
    if (!val || etiologicalFactors.includes(val)) return;
    onChange([...etiologicalFactors, val]);
    setInputValue("");
  };

  const handleRemove = (item: string) => {
    onChange(etiologicalFactors.filter(f => f !== item));
  };
    ...
    <div className="bg-slate-900 border border-slate-850 p-5 rounded-xl space-y-4">
      <div>
        <h4 className="text-xs font-black uppercase text-slate-200 tracking-wider flex items-center gap-2">
          <Tag className="w-4 h-4 text-emerald-405" /> Etiological Factors (Causation)
        </h4>
        <p className="text-[10px] text-slate-500 mt-1">Record triggers or underlying causes of the pathology (e.g., "grief", "wet cold weather", "injury", "suppression").</p>
      </div>
      ...
      <form onSubmit={handleAdd} className="flex gap-2">
        <input
          ...
          placeholder="Add cause or trigger e.g. exposure to dry wind..."
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        {etiologicalFactors.length > 0 ? (
          etiologicalFactors.map(f => (
            <span
              key={f}
          ...
          <p className="text-xs text-slate-550 italic">No etiological factors logged yet.</p>
        )}
      </div>
    </div>
  );
}
export default EtiologicalFactorsSection;
