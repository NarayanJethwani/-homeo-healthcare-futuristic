...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  const loadChunk=async(ci:number)=>{
   const chunk=atlas.chunks[ci];if(typeof DecompressionStream==='undefined')throw new Error('This browser cannot open compressed anatomy. Use the simple regional map.');const compressed=true;const response=await fetch(chunk.gzip!,{signal:abort.signal});const buffer=await decodeModelResponse(response,chunk.bytes,compressed);if(disposed)return;
  ...
  const fit=(view:string,extent=0)=>{
   const normalDistance=4.2;
   const availableAspect=Math.max(.35,el.clientWidth/Math.max(1,el.clientHeight)*.9);const atlasDistance=Math.max(packingHeight,packingWidth/availableAspect)/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))*1.15;
   ...
   controls.target.set(extent>.1?-packingWidth*.12:0,.85,0);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
  };
  ...
  let lastFrame=performance.now(),lastExtent=-1;
  const animate=()=>{
   if(disposed)return;frame=requestAnimationFrame(animate);const now=performance.now(),dt=Math.min((now-lastFrame)/1000,.05),s=latest.current;lastFrame=now;
 ...
 return <div className="absolute inset-0 [&_canvas]:h-full [&_canvas]:w-full [&_canvas]:touch-none [&_.part-hover]:pointer-events-none [&_.part-hover]:absolute [&_.part-hover]:z-10 [&_.part-hover]:rounded-lg [&_.part-hover]:bg-slate-950/90 [&_.part-hover]:px-2.5 [&_.part-hover]:py-1.5 [&_.part-hover]:text-xs [&_.part-hover]:text-white" ref={host}/>;
}
