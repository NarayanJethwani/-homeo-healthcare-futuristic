...
import { Calendar, User, Clock, ArrowRight, ClipboardCheck, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function TodaySchedule({
  ...
  const getPriorityClass = (priority: string) => {
    switch (priority) {
      case "Critical":
        return "bg-rose-50 border-rose-200 dark:bg-rose-955/20 dark:border-rose-900/30 text-rose-650 dark:text-rose-400";
      case "Important":
        return "bg-amber-50 border-amber-200 dark:bg-amber-955/20 dark:border-amber-900/30 text-amber-700 dark:text-amber-400";
      default:
        return "bg-blue-50 border-blue-200 dark:bg-blue-955/20 dark:border-blue-900/30 text-blue-650 dark:text-blue-400";
    }
  };
  ...
  const getStatusClass = (status: string) => {
    switch (status) {
      case "Waiting":
        return "bg-amber-100 text-amber-805 dark:bg-amber-900/40 dark:text-amber-350";
      case "Completed":
        return "bg-emerald-100 text-emerald-805 dark:bg-emerald-900/40 dark:text-emerald-350";
      default:
        return "bg-sky-105 text-sky-805 dark:bg-sky-900/40 dark:text-sky-350";
    }
  };
  ...
  if (error) {
    return (
      <div className="bg-rose-50 dark:bg-rose-955/20 border border-rose-200 dark:border-rose-900/60 p-6 rounded-3xl flex items-center justify-between select-none">
      ...
      {appointments.length > 0 ? (
        <div className="overflow-x-auto select-text">
          <table className="w-full text-left text-xs divide-y divide-slate-100 dark:divide-slate-800">
            <thead>
              <tr className="text-slate-400 dark:text-slate-500 font-extrabold uppercase text-[9px] tracking-wider">
                ...
                <tr key={apt.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="py-3.5 pl-2 font-bold text-slate-850 dark:text-slate-100">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 flex items-center justify-center shrink-0">
                        <User className="w-3.5 h-3.5" />
                      </div>
                      <div className="truncate">
                        <div>{apt.name}</div>
                        {apt.age && (
                          <span className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">{apt.age} y/o</span>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="py-3.5 text-slate-550 dark:text-slate-400 font-mono font-bold">
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-slate-400" />
                      <span>{apt.time}</span>
                    </div>
                  </td>
                  <td className="py-3.5 text-slate-550 dark:text-slate-400 max-w-xs truncate">
                    {apt.purpose}
                  </td>
                  <td className="py-3.5">
                  ...
                  <td className="py-3.5 pr-2 text-right">
                    <button
                      ...
                      className={`px-3 py-1 bg-slate-50 dark:bg-slate-800 hover:bg-teal-50 dark:hover:bg-teal-950/30 text-slate-600 dark:text-slate-400 hover:text-teal-650 dark:hover:text-teal-400 border border-slate-200 dark:border-slate-700 hover:border-teal-200 dark:hover:border-teal-900/50 rounded-xl text-[10px] font-bold cursor-pointer inline-flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-teal-500 outline-none transition-all ${
        ...
        <div className="p-8 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl space-y-2">
          ...
          <p className="text-[10px] text-slate-400 dark:text-slate-600 max-w-sm mx-auto leading-relaxed">
