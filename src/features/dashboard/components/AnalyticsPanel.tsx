...
import { TrendingUp, BarChart4, DollarSign, Percent, ThumbsUp, Activity, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function AnalyticsPanel({
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] border border-slate-200/80 dark:border-slate-800/80 shadow-xs animate-pulse space-y-6">
        <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
          <div className="h-4 w-32 bg-slate-200 dark:bg-slate-800 rounded" />
          <div className="h-6 w-24 bg-slate-250 dark:bg-slate-850 rounded" />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 h-40 bg-slate-100 dark:bg-slate-850/50 rounded-3xl" />
          <div className="lg:col-span-4 space-y-4">
            <div className="h-4 w-2/3 bg-slate-200 dark:bg-slate-800 rounded" />
            <div className="h-3 w-5/6 bg-slate-150 dark:bg-slate-850 rounded" />
            <div className="h-3 w-4/6 bg-slate-150 dark:bg-slate-850 rounded" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-6 rounded-[32px] border border-slate-200/80 dark:border-slate-800/80 shadow-xs space-y-6 select-text">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-3 select-none">
        <h3 className="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
          <BarChart4 className="w-4.5 h-4.5 text-teal-500" />
          <span>Clinical & Financial Analytics</span>
        </h3>
        
        {/* Timeframe selector */}
        <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl gap-1 max-w-fit border border-slate-100 dark:border-slate-750">
          {(["today", "week", "month", "year"] as const).map((tf) => (
            <button
              key={tf}
              onClick={() => setTimeframe(tf)}
              className={`px-3 py-1 rounded-lg text-[9.5px] font-bold uppercase transition-all border-none cursor-pointer focus-visible:ring-1 focus-visible:ring-teal-555 outline-none ${
                timeframe === tf
        ...
        <div className="lg:col-span-8 space-y-4">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-[10.5px] font-bold transition-all flex items-center gap-1.5 cursor-pointer border focus-visible:ring-2 focus-visible:ring-teal-500 outline-none ${
                    activeTab === tab.id
                      ? "bg-teal-50/50 dark:bg-teal-955/20 border-teal-200 dark:border-teal-900 text-teal-655 dark:text-teal-400"
                      : "bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-800 dark:hover:text-slate-200"
                  }`}
                >
          ...
          <div className="w-full bg-slate-50 dark:bg-slate-850/50 border border-slate-200/50 dark:border-slate-800 rounded-3xl p-4 flex flex-col justify-between">
            <svg
              ...
              role="img"
              aria-label={`Analytics chart rendering ${activeTab}`}
            >
              {/* Grid Lines */}
            ...
            <div className="flex items-center justify-between text-[9px] text-slate-400 dark:text-slate-500 font-mono mt-3 px-1 border-t border-slate-150/40 dark:border-slate-800 pt-2 shrink-0 select-none">
              ...
              {activeTab === "revenue" && (
                <span>Total Revenue: <span className="text-sky-555 font-bold">₹8.4L</span></span>
              )}
            </div>
          </div>
        </div>

        {/* Right Column: Mini Stats Summary */}
        <div className="lg:col-span-4 space-y-5">
          <div className="space-y-4">
            {/* Disease distribution */}
            <div className="space-y-1">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-slate-405 dark:text-slate-500 block">
                Disease Affinity
              </span>
                ...
                {[
                  ...
                  <div key={idx} className="space-y-1 text-xs">
                    ...
                    <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                      <div className={`h-full ${item.color} ${reduceMotion ? "" : "transition-all duration-500"}`} style={{ width: `${item.pct}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-1 pt-2">
              ...
              <div className="flex flex-wrap gap-2 pt-1.5">
                {[
                  ...
                  <span
                    key={idx}
                    className="bg-slate-50 dark:bg-slate-850 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-205 dark:border-slate-800 px-2.5 py-1 rounded-xl text-[10px] font-bold text-slate-600 dark:text-slate-400 flex items-center gap-1.5 select-none"
                  >
                    <span>{rem.name}</span>
            ...
            {/* Compliance Stats indicators */}
            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-100 dark:border-slate-800 select-none">
              <div className="p-3 bg-slate-50 dark:bg-slate-850/50 rounded-2xl text-center space-y-1 border border-slate-100 dark:border-slate-800">
                ...
                <div className="text-sm font-bold text-slate-855 dark:text-slate-100">
                  88.5%
                </div>
              </div>
              <div className="p-3 bg-slate-50 dark:bg-slate-855/50 rounded-2xl text-center space-y-1 border border-slate-100 dark:border-slate-800">
