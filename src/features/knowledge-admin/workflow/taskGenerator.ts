...
import { EditorialTask, EditorialTaskPriority, EditorialTaskStatus, EditorialTaskType } from "./types";
import { createEditorialTask, getEditorialTasks } from "./workflowManager";
import { getEntityOsUsageCounts } from "../../knowledge/analytics/clinicalOsKnowledgeUsage";
 ...
 */
export async function generateAutomaticCurationTasks(entities: KmsKnowledgeEntity[], actor: string = "System Task Generator"): Promise<EditorialTask[]> {
  const generated: EditorialTask[] = [];
  const now = new Date();
  ...
  for (const entity of entities) {
    ...
    const osUsage = getEntityOsUsageCounts(entity.id);
    ...
    if (needsClinicalReview) {
      ...
      const tPriority: EditorialTaskPriority = isCornerstone ? "critical" : (isHighUsage ? "high" : "medium");
    ...
    if (needsReferenceUpdate) {
      ...
      const tPriority: EditorialTaskPriority = (isCornerstone || isCriticalHealth) ? "critical" : "medium";
      const task = await createEditorialTask({
        articleId: entity.id,
        articleTitle: title,
        entityType: entity.entityType,
