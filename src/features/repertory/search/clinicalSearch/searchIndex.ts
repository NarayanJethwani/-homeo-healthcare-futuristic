...
import { SearchField, SearchIndex, SearchIndexDocument, SearchableField } from "./types";
...
function joinValues(values: Array<string | string[] | undefined>): string {
  return values
...
function remedyText(rubric: CanonicalRubric): string {
...
function buildField(field: SearchField, text: string): SearchableField | null {
...
function clinicalPriorityWeight(priority: unknown): number {
  if (priority === "high") return 1.3;
  if (priority === "medium") return 1.1;
  if (priority === "low") return 0.95;
  return 1;
}

function buildDocument(rubric: CanonicalRubric): SearchIndexDocument {
  const fieldCandidates: Array<[SearchField, string]> = [
    ["title", rubric.title],
  ...
  const fields = fieldCandidates
    ...
    .filter((field): field is SearchableField => field !== null);
...
export function buildCanonicalSearchIndex(rubrics: CanonicalRubric[], builtAt = new Date().toISOString()): SearchIndex {
  const documents = rubrics.map(buildDocument);
  const tokenToRubricIds = new Map<string, Set<string>>();

  documents.forEach((document) => {
    document.allTokens.forEach((token) => {
      if (!tokenToRubricIds.has(token)) tokenToRubricIds.set(token, new Set());
