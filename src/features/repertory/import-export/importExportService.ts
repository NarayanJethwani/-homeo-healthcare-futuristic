...
import { RepertoryRubric, GraphTriple, GradedRemedy } from '../types';
...
export class ImportExportService {
   ...
   */
  static async exportToMDX(): Promise<string> {
    const rubrics = await repertoryRepository.getRubrics();
   ...
   */
  static async exportToGraphTriples(): Promise<string> {
    const triples = await repertoryRepository.getTriples();
    let result = '';
   ...
   */
  static async importRubricsFromJSON(jsonString: string): Promise<boolean> {
    try {
      ...
      if (data.rubrics && Array.isArray(data.rubrics)) {
        for (const rub of data.rubrics) {
          ...
          if (rub.rubricId && rub.title && rub.category) {
            await repertoryRepository.saveRubric(rub);
          }
        }
      }
      ...
      if (data.triples && Array.isArray(data.triples)) {
        for (const tr of data.triples) {
          if (tr.subjectId && tr.predicate && tr.objectId) {
            await repertoryRepository.saveTriple(tr);
          }
        }
      }
   ...
   */
  static async importRubricsFromCSV(csvString: string): Promise<boolean> {
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const category = matches[4].replace(/"/g, '') as any;
        ...
        const relatedRemedies: GradedRemedy[] = remediesRaw.split(';').map(rem => {
          const parts = rem.split(':');
          const remedyId = parts[0] || 'Unknown';
          const grade = parts[1] ? Number(parts[1]) as any : 1;
        ...
        await repertoryRepository.saveRubric(rubric);
      }
