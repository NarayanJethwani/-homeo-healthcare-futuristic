import { getRemedyGradeWeight, RemedyGrade } from "../../engine/canonicalTypes";
import { getScoringStrategy } from "./scoringStrategies";
import {
...
function isExcluded(session: ClinicalRepertorizationSession, rubricId: string, remedyId: string): boolean {
  return session.exclusions.rubricIds.includes(rubricId) || session.exclusions.remedyIds.includes(remedyId);
}

function remedyFrequency(session: ClinicalRepertorizationSession): Map<string, number> {
  const frequency = new Map<string, number>();

  session.selectedRubrics.forEach((selectedRubric) => {
    selectedRubric.rubric.remedies.forEach((rubricRemedy) => {
      if (rubricRemedy.isEliminating || isExcluded(session, selectedRubric.rubric.id, rubricRemedy.remedyId)) return;
...
function maxPossibleWeightedScore(session: ClinicalRepertorizationSession): number {
  return session.selectedRubrics.reduce((sum, selectedRubric) => (
    sum + (4 * selectedRubric.rubricWeight * selectedRubric.symptomImportance)
  ), 0);
}

function contributionPercentage(contribution: RubricContribution, total: number): number {
  if (total <= 0) return 0;
  return Math.round((contribution.strategyContribution / total) * 10000) / 100;
}

function whyRanked(ranking: Omit<RemedyRanking, "whyRanked">): string[] {
...
function buildContext(session: ClinicalRepertorizationSession): ScoringContext {
  return {
    session,
    remedyFrequencyById: remedyFrequency(session),
    maxPossibleGrade: 4 as RemedyGrade,
  };
}

function buildContribution(
  selectedRubric: ClinicalRepertorizationSession["selectedRubrics"][number],
  remedyId: string,
  strategyContribution: number,
): RubricContribution | null {
  const rubricRemedy = selectedRubric.rubric.remedies.find((remedy) => remedy.remedyId === remedyId);
  if (!rubricRemedy) return null;

  const gradeContribution = getRemedyGradeWeight(rubricRemedy.grade);
...
export function repertorizeClinicalSession(
  session: ClinicalRepertorizationSession,
  customStrategies: ScoringStrategy[] = [],
  generatedAt = new Date().toISOString(),
): ClinicalRepertorizationResult {
  const strategy = getScoringStrategy(session.strategyId, customStrategies);
  const context = buildContext(session);
  const contributionMap = new Map<string, RubricContribution[]>();
  ...
  session.selectedRubrics.forEach((selectedRubric) => {
    selectedRubric.rubric.remedies.forEach((rubricRemedy) => {
      ...
      if (!contributionMap.has(rubricRemedy.remedyId)) contributionMap.set(rubricRemedy.remedyId, []);
  ...
  const rankings: RemedyRanking[] = Array.from(contributionMap.entries()).map(([remedyId, contributions]) => {
    const totalScore = contributions.reduce((sum, contribution) => sum + contribution.strategyContribution, 0);
    const weightedScore = contributions.reduce((sum, contribution) => (
      sum + (contribution.gradeContribution * contribution.weightContribution)
    ), 0);
