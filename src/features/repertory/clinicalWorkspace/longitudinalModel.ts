...
export class LongitudinalCaseModel {
  ...
  public static buildLongitudinalSummary(
    ...
    let responseTrend: LongitudinalCaseSummary['responseTrend'] = 'stable';
    ...
    if (prevVisit) {
      ...
      if (skinImproved && internalWorsened) {
        ...
        suppressionWarnings.push(
          "Hering's Law Alert: Possible suppression detected! Superficial skin eruptions resolved/improved, but deeper internal respiratory or mental anxiety symptoms have worsened."
        );
      }
    }

    // 2. Relapse Indicators
    const relapseIndicators: string[] = [];
      ...
      if (history.length >= 3) {
        ...
        if (middle < initial && current > middle) {
          relapseIndicators.push(
            `Relapse warning: "${trend.rubricTitle}" was improving (intensity ${middle}) but has flared up again (intensity ${current}).`
          );
        }
      }
    });
    ...
    if (prevVisit) {
      const newSymptoms = lastVisit.activeSymptoms.filter(
        curr => !prevVisit.activeSymptoms.some(prev => prev.rubricId === curr.rubricId)
      );
      if (newSymptoms.length >= 2) {
        unexpectedFindings.push(
          `Unexpected shift: ${newSymptoms.length} new symptoms appeared in the current visit: ${newSymptoms.map(s => rubricTitles[s.rubricId] || s.rubricId).join(', ')}.`
        );
      }
    }
