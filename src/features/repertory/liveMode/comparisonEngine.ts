...
import { V2RubricSnapshot } from "./types";

function rankMap(ids: string[]): Map<string, number> {
  return new Map(ids.map((id, index) => [id, index + 1]));
}

export function compareRubricSnapshots(v1Rubrics: V2RubricSnapshot[], v2Rubrics: V2RubricSnapshot[]) {
...
export function compareRemedyScores(
  v1Rankings: Array<Pick<RemedyRanking, "remedyId" | "totalScore">>,
  v2Rankings: Array<Pick<RemedyRanking, "remedyId" | "totalScore">>,
) {
