...
export interface VectorIndexStats {
  ...
  dimensions?: number;
  model?: string;
  source: "static-seed" | "memory" | "firestore" | "gcs" | "persistent";
...
export interface PersistentVectorStore {
  getVector(id: string): Promise<VectorRecord | null>;
  getVectors(ids: string[]): Promise<VectorRecord[]>;
  upsertVector(record: VectorRecord): Promise<void>;
  deleteVector(id: string): Promise<void>;
  getIndexStats(): Promise<VectorIndexStats>;
  getStats(): Promise<VectorIndexStats>;
  listStaleVectors(): Promise<VectorRecord[]>;
  loadSeedVectors(): Promise<VectorRecord[]>;
}
...
export class MemoryVectorStore implements PersistentVectorStore {
  protected cache = new Map<string, VectorRecord>();
  protected seedLoaded = false;
  protected dimensions = 768; // Default to nomic-embed-text
  protected modelName = "nomic-embed-text";
  protected dimensionMismatches = 0;
  protected staticFilePath: string;

  constructor() {
    this.staticFilePath = path.join(
      process.cwd(),
      "src/features/knowledge/retrieval/vectors.json"
    );
  }

  async loadSeedVectors(): Promise<VectorRecord[]> {
    if (this.seedLoaded) {
    ...
    try {
      if (fs.existsSync(this.staticFilePath)) {
        const fileContent = await fs.promises.readFile(this.staticFilePath, "utf-8");
        const records = JSON.parse(fileContent) as VectorRecord[];
  ...
  async getVectors(ids: string[]): Promise<VectorRecord[]> {
  ...
  async getIndexStats(): Promise<VectorIndexStats> {
    if (!this.seedLoaded) {
      await this.loadSeedVectors();
    }
    return {
  ...
  async listStaleVectors(): Promise<VectorRecord[]> {
    return [];
  }
}

export class FirestoreVectorStore implements PersistentVectorStore {
  private memoryFallback = new MemoryVectorStore();
  ...
  async getVector(id: string): Promise<VectorRecord | null> {
    ...
    try {
      ...
      if (doc.exists) {
        return doc.data() as VectorRecord;
      }
    } catch (err: any) {
  ...
  async getVectors(ids: string[]): Promise<VectorRecord[]> {
    ...
    try {
      ...
      for (const chunk of chunks) {
        ...
        snap.forEach((doc: any) => results.push(doc.data() as VectorRecord));
      }
  ...
  async getIndexStats(): Promise<VectorIndexStats> {
    ...
    try {
      ...
      if (!snap.empty) {
        const first = snap.docs[0].data() as VectorRecord;
...
export class HybridPersistentVectorStore implements PersistentVectorStore {
  private firestoreStore = new FirestoreVectorStore();
  private memoryStore = new MemoryVectorStore();
  ...
  async getIndexStats(): Promise<VectorIndexStats> {
    ...
    try {
      const { globalKmsRepository } = await import("../../knowledge-admin/repositories/MemoryRepository");
...
export const globalVectorStore = new HybridPersistentVectorStore();
