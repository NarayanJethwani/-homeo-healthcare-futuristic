...
export default function KnowledgeGraphExplorer({ currentId }: KnowledgeGraphExplorerProps) {
  ...
  // Solid for direct, dashed for complementary, dotted for compare/fallbacks
  ...
  const radiusPercent = 38; // Slightly larger radius for standard look
  ...
  const mainContent = (
    <div className="relative w-full h-full flex items-center justify-center bg-neutral-950/20 dark:bg-black/10 rounded-2xl overflow-hidden border border-neutral-500/5 select-none">
        ...
        {nodes.map((node, idx) => {
          ...
          return (
            <line
              ...
              x2={nx}
              y2={ny}
              className="stroke-neutral-300 dark:stroke-neutral-800 transition-all duration-300"
              strokeWidth={hoveredNode?.id === node.id ? "0.8" : "0.3"}
      ...
      {nodes.map((node, idx) => {
        ...
        return (
          <Link
            ...
            className={`absolute -translate-x-1/2 -translate-y-1/2 p-2 border rounded-2xl text-center flex flex-col items-center justify-center gap-0.5 shadow-sm transition-all duration-300 hover:scale-105 z-10 w-24 md:w-28 h-12 leading-none cursor-pointer ${getThemeColor(
              node.type
            )}`}
          >
      ...
      {hoveredNode && (
        <div className="absolute bottom-4 left-4 right-4 z-40 p-3.5 bg-neutral-900/95 dark:bg-black/95 border border-neutral-800 rounded-2xl text-left backdrop-blur-xl animate-fadeIn pointer-events-none">
          <div className="flex items-center justify-between gap-2 mb-1.5">
            <h6 className="text-[10px] font-bold text-neutral-50 flex items-center gap-1">
              {getIcon(hoveredNode.type)} {hoveredNode.title}
            </h6>
