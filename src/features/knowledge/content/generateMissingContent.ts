...
ADDITIONAL_REMEDIES.forEach(name => {
  if (remediesToGenerate.length >= 97) return;
  remediesToGenerate.push({
    id: `R${String(remedyIdCounter++).padStart(4, "0")}`,
    name: name,
    common: `${name} Common`,
...
const generatedRemediesExports: string[] = [];
...
const generatedDiseasesExports: string[] = [];
DISEASE_NAMES.forEach(name => {
  if (generatedDiseasesExports.length >= 46) return;
...
const generatedSymptomsExports: string[] = [];
SYMPTOM_NAMES.forEach(name => {
  if (generatedSymptomsExports.length >= 47) return;
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
  const exportName = name.replace(/[^a-zA-Z0-9]+/g, "");
...
const generatedLabTestsExports: string[] = [];
LAB_TEST_NAMES.forEach(name => {
  if (generatedLabTestsExports.length >= 23) return;
  const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
  const exportName = name.replace(/[^a-zA-Z0-9]+/g, "");
