...
export interface AuditIssue {
  ...
  title: string;
  type: "error" | "warning";
  category: "content" | "relationship" | "citation" | "formatting";
  message: string;
}
...
export interface EditorialAuditSummary {
  ...
  issues: AuditIssue[];
  brokenRelationships: { sourceId: string; targetId: string; type: string }[];
  missingCitations: { entityId: string; citationId: string }[];
}

export function runEditorialAudit(): EditorialAuditSummary {
  ...
  const brokenRelationships: { sourceId: string; targetId: string; type: string }[] = [];
  const missingCitations: { entityId: string; citationId: string }[] = [];

  let healthyCount = 0;

  // Track relationship counts per entity
  const outboundLinkCountMap = new Map<string, number>();
  const inboundLinkCountMap = new Map<string, number>();
