...
const REQUIRED_LOCALES: Locale[] = ["en"]; // English is mandatory. Others are optional/fallback.
...
export function validateQualityGates(
  ...
  const titleClaims = checkProhibitedClaims(entity.title);
  const summaryClaims = checkProhibitedClaims(entity.summary);
  const contentClaims = checkProhibitedClaims(entity.content);
  ...
  const hasDisclaimerTerm = 
    contentStr.includes("disclaimer") || 
