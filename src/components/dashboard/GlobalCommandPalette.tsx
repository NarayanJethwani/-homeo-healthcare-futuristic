...
import { Search, User, FileText, Cpu, Settings, Sparkles, BookOpen, AlertCircle } from "lucide-react";
...
export default function GlobalCommandPalette({
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="fixed inset-x-4 top-[12%] mx-auto max-w-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl z-[101] overflow-hidden flex flex-col max-h-[60vh] select-none text-slate-800 dark:text-slate-200 animate-in fade-in zoom-in-95 duration-150"
      >
        {/* Input area */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
          ...
          <input
            ...
            placeholder="Search patient, remedy code, invoice, page setting, action... (Use ↑↓ Enter)"
            autoFocus
            value={query}
            onChange={(e) => setQuery(e.target.value)}
          />
          <kbd className="hidden sm:inline-block px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-[9px] font-mono text-slate-400">
            ESC
          </kbd>
        </div>
        ...
        >
          {searchResults.length > 0 ? (
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-2xl cursor-pointer transition-all ${
                    isSelected
                      ? "bg-slate-50 dark:bg-slate-850 border border-emerald-250 dark:border-emerald-900 text-emerald-600 dark:text-emerald-400 shadow-xs"
                      : "border border-transparent hover:bg-slate-50/50 dark:hover:bg-slate-800/50"
                  }`}
                >
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                      isSelected
                        ? "bg-emerald-100/50 dark:bg-emerald-950/40 text-emerald-650 dark:text-emerald-400"
                  ...
                  <div className="text-[10px] uppercase font-bold tracking-widest text-slate-350 dark:text-slate-650">
                    {item.type}
                  </div>
                </div>
              );
            ...
            <div className="p-8 text-center space-y-2">
              ...
              <div className="text-[10px] text-slate-350 dark:text-slate-700">
