...
export interface SearchFilterTokens {
 ...
 */
export function parseNaturalLanguageQuery(query: string): SearchFilterTokens {
  const tokens: SearchFilterTokens = { symptomKeywords: [] };
  const lower = query.toLowerCase();

  // 1. Kingdom Identifiers
  if (lower.includes("plant") || lower.includes("botanical") || lower.includes("vegetable")) tokens.kingdom = "Plant";
  if (lower.includes("mineral") || lower.includes("chemical") || lower.includes("element")) tokens.kingdom = "Mineral";
  if (lower.includes("animal") || lower.includes("venom") || lower.includes("snake") || lower.includes("spider")) tokens.kingdom = "Animal";
  if (lower.includes("nosode") || lower.includes("bacterial") || lower.includes("diseased")) tokens.kingdom = "Nosode";

  // 2. Thermal Identifiers
  if (lower.includes("chilly") || lower.includes("cold sensitivity") || lower.includes("worse cold")) tokens.thermal = "Chilly";
  if (lower.includes("hot-blooded") || lower.includes("warmth of bed") || lower.includes("worse warmth")) tokens.thermal = "Hot";

  // 3. Lateralization Identifiers
  if (lower.includes("right-sided") || lower.includes("right side") || lower.includes("right-side")) tokens.lateralization = "Right";
  if (lower.includes("left-sided") || lower.includes("left side") || lower.includes("left-side")) tokens.lateralization = "Left";

  // 4. Miasm Identifiers
  if (lower.includes("psora") || lower.includes("psoric")) tokens.miasm = "Psora";
  if (lower.includes("sycosis") || lower.includes("sycotic")) tokens.miasm = "Sycosis";
  if (lower.includes("syphilis") || lower.includes("syphilitic")) tokens.miasm = "Syphilis";
  if (lower.includes("tubercular")) tokens.miasm = "Tubercular";
  if (lower.includes("cancerinic") || lower.includes("carcinosin")) tokens.miasm = "Cancerinic";
 ...
 */
export function searchRemediesAdvanced(filters: SearchFilterTokens): AdvancedSearchResult[] {
  const results: AdvancedSearchResult[] = [];

  GENOME_REMEDY_DB.forEach(rem => {
    let score = 0;
    const reasons: Array<{ facet: string; matchText: string; score: number }> = [];
    ...
    if (filters.miasm) {
      let miasmScore = 0;
      if (filters.miasm === "Psora") miasmScore = rem.genome.psoricDrive;
      else if (filters.miasm === "Sycosis") miasmScore = rem.genome.sycoticDrive;
      else if (filters.miasm === "Syphilis") miasmScore = rem.genome.syphiliticDrive;
      else if (filters.miasm === "Tubercular") miasmScore = rem.genome.tubercularDrive;
      else if (filters.miasm === "Cancerinic") miasmScore = rem.genome.cancerinicDrive;
    ...
    filters.symptomKeywords.forEach((keyword: string) => {
