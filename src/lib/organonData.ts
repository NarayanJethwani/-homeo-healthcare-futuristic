...
export interface OrganonEdition {
  id: number;
  name: string;
  year: string;
  aphorismsCount: number;
  significance: string;
  keyShifts: string[];
}

export interface DiseaseAffinity {
  migraine: string;
  asthma: string;
  psoriasis: string;
  anxiety: string;
...
export interface OrganonAphorism {
  ...
  repertoryLink: string;
  dailyPracticeExample: string;
  crossReferences: string[];
  relatedConcepts: string[];
  relatedRemedies: string[];
  affinities: DiseaseAffinity;
}

export interface OrganonCase {
  id: string;
  title: string;
  principle: string; // "Single Remedy", "Minimum Dose", etc.
...
export interface ActiveRecallQuestion {
  id: string;
  type: 'mcq' | 'flashcard' | 'true-false' | 'match';
  difficulty: 'beginner' | 'student' | 'practitioner' | 'advanced' | 'teacher';
  category: 'BHMS' | 'MD' | 'Competitive' | 'Faculty';
  question: string;
  options?: string[]; // For MCQs
  correctAnswer: string; // Correct index for MCQs, "True"/"False" for TF, Back text for flashcard
  explanation: string;
}
...
export interface TimelineStep {
  ...
  sixthEditionStance: string;
}

// 1. EDITIONS
export const ORGANON_EDITIONS: OrganonEdition[] = [
  {
    id: 1,
...
// 2. KNOWLEDGE TREE
export const ORGANON_KNOWLEDGE_TREE = [
  {
...
// 3. APHORISMS DATA
export const ORGANON_APHORISMS: OrganonAphorism[] = [
  {
    id: "1",
    number: "§1",
    edition: "6th Edition",
...
export const ORGANON_CASES: OrganonCase[] = [
  {
    id: "case_01",
...
export const ACTIVE_RECALL_EXERCISES: ActiveRecallQuestion[] = [
  {
    id: "ar_01",
    type: "mcq",
    difficulty: "beginner",
