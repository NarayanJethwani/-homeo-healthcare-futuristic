import { getSimulatedCase } from "./caseIntelligence";
...
export interface VirtualPatient {
  ...
  caseHistory: string;
  hiddenAffinities: string[];
  correctRubrics: string[];
  correctRemedyId: string;
  correctPotency: string;
  miasmaticBase: string;
}

export interface StudentEvaluation {
  patientId: string;
  extractedRubrics: string[];
  selectedRemedyId: string;
  selectedPotency: string;
  isCorrectRemedy: boolean;
  isCorrectPotency: boolean;
  score: number;
  feedback: {
    rubricRelevance: string;
    remedyJustification: string;
    potencyJustification: string;
    heringsDirectionAnalysis: string;
  };
}
 ...
 */
export const VIRTUAL_PATIENTS: VirtualPatient[] = [
  {
    id: "pat_psoric_child",
 ...
 */
export function evaluateCaseSubmission(
  patientId: string,
  submission: { selectedRemedyId: string; selectedPotency: string; extractedRubrics: string[] }
): StudentEvaluation {
  let patient: VirtualPatient | undefined;

  if (patientId.startsWith("case_")) {
    const idx = parseInt(patientId.split("_")[1], 10) - 1;
    const simCase = getSimulatedCase(idx);
    let miasm = "Psora";
    if (simCase.correctRemedyId === "rem_lycopodium") miasm = "Psora-Sycosis";
    else if (simCase.correctRemedyId === "rem_nat_mur") miasm = "Sycosis-Syphilis";
    else if (simCase.correctRemedyId === "rem_arsenicum") miasm = "Psora-Syphilis";
    else if (simCase.correctRemedyId === "rem_lachesis") miasm = "Sycosis";
    else if (simCase.correctRemedyId === "rem_calcarea") miasm = "Tubercular";
    else if (simCase.correctRemedyId === "rem_pulsatilla") miasm = "Psora";
    else if (simCase.correctRemedyId === "rem_gelsemium") miasm = "Psora";
    else if (simCase.correctRemedyId === "rem_bryonia") miasm = "Psora";
    ...
    patient = VIRTUAL_PATIENTS.find(p => p.id === patientId);
  }
  ...
  patient.correctRubrics.forEach((rub: string) => {
    // Check if any selected rubric contains the keyword
    const key = rub.toLowerCase();
    const hasMatch = Array.from(submissionRubricsSet).some(subRub => 
      subRub.includes(key) || key.includes(subRub)
    );
    if (hasMatch) matchedRubricCount++;
  });
  ...
  let score = 0;
  if (isCorrectRemedy) score += 50;
  if (isCorrectPotency) score += 20;
  ...
  if (matchedRubricCount === patient.correctRubrics.length) {
    ...
    rubricFeedback = `Partially correct. You mapped ${matchedRubricCount} out of ${patient.correctRubrics.length} keynotes. You missed: ${patient.correctRubrics.filter((r: string) => !submission.extractedRubrics.includes(r)).join(", ")}.`;
  } else {
