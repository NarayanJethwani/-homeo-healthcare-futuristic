...
export interface GraphNode {
  ...
  metadata?: {
    description?: string;
    profile?: RemedyMasterProfile; // For remedies
    pathology?: string;           // For conditions
    origin?: string;              // For families/kingdoms
    rubricCode?: string;          // For rubrics
    aggFactors?: string[];        // For modalities
    miasmaticExpression?: string;  // For miasms
  };
}
...
export interface GraphEdge {
  ...
  target: string;
  type: EdgeType;
  weight: number; // intensity grade (1, 2, 3) or match strength
}

export interface HomeopathicKnowledgeGraph {
  nodes: GraphNode[];
  edges: GraphEdge[];
}
...
export const getKnowledgeGraph = (): HomeopathicKnowledgeGraph => {
  const nodes = [...KNOWLEDGE_GRAPH_NODES];
  const edges = [...KNOWLEDGE_GRAPH_EDGES];
  ...
  MASTER_REMEDY_DB.forEach(remedy => {
    const rid = remedy.id;
    
    const profile: RemedyMasterProfile = {
    ...
    const miasmKeys: ('psora' | 'sycosis' | 'syphilis' | 'tubercular' | 'cancerinic')[] = ['psora', 'sycosis', 'syphilis', 'tubercular', 'cancerinic'];
    ...
    const connectRel = (relName: string, type: 'complementary' | 'inimical' | 'follows_well') => {
      const targetId = resolveCanonicalRemedyId(relName);
...
export const getRemedyProfile = (remedyId: string): RemedyMasterProfile | undefined => {
  const node = getKnowledgeGraph().nodes.find(n => n.id === remedyId && n.type === 'remedy');
