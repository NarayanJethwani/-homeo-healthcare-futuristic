...
- **Major Changes**:
  ...
  - **Analytics Dashboard UI**: Upgraded the admin page at `/admin/observability` to render metric panels showing live cache outcomes, provider attempts, and latency distribution buckets. Added a metrics reset button utilizing a browser native confirmation dialog protected by exact-origin CORS and Origin/Host matching.
